...
class CreditCard extends AbstractSubscriber
{
    ...
    public function onPostDispatchFrontendCheckout(Enlight_Controller_ActionEventArgs $args)
    {
        ...
        if (!$request->isDispatched() or !stristr($paymentName, 'fatchip_computop_creditcard')) { // no creditcard payment method
            return;
        }
        ...
        if ($request->getActionName() == 'confirm' and $pluginConfig['creditCardMode'] == 'SILENT' and !isset($session->FatchipCTBrowserInfoParams['javaScriptEnabled'])) {
            ...
            $view->loadTemplate('frontend/checkout/computop_creditcard_confirm_browserdetect.tpl');
